...
def write(input,path):
    input = input.reshape(-1).cpu().numpy()
...
def read(path):
    ...
    with open(path,'r') as f:
        ...
        for i in lines[0].split():
            input.append(int(i))
    input = torch.Tensor(input).to(torch.int64).cuda()
    return input
        ...
        for _, batch in enumerate(dataloader):
            ...
            #False
                ...
                if args.pretrained_bert:
                    ...
                    # write(inputs[0],"/dataset/lichunyou/GLM/input/a")
                    # write(inputs[1],"/dataset/lichunyou/GLM/input/b")
                    # write(inputs[2],"/dataset/lichunyou/GLM/input/c")
                    # write(inputs[3],"/dataset/lichunyou/GLM/input/d")
                    # write(inputs[4],"/dataset/lichunyou/GLM/input/e")
                    ...
                    inputs = [inputs_a  for i in range(500)]
                    b = time.time()
                    for i in range(len(inputs)):
                         logits, *mems = model(*inputs[i])
                    e = time.time()
                    print(e-b)
                    exit(0)
